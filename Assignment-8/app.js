  ...
  if(!products || !categories){
    try{
      ...
      products = apiProducts.map(p => {
        ...
        if(!catMap[catName]){
          catMap[catName] = { id: uid(), name: catName, image: p.thumbnail || "" };
        }
        ...
        return {
          ...
          images: p.images?.length ? p.images : [p.thumbnail || "https://via.placeholder.com/400"],
...
function getCategories(){ return readJSON("categories") || []; }
function saveProducts(prods){ writeJSON("products", prods); }
function saveCategories(cats){ writeJSON("categories", cats); }
...
function addProduct({title, price, description, categoryId, images}){
  ...
  const newProd = {
    ...
    category: cat ? {id: cat.id, name: cat.name} : {id:null, name:"Uncategorized"}
  };
...
/* ---------- Cart ---------- */
function getCart(){ return readJSON("cart")||[]; }
function saveCart(cart){ writeJSON("cart", cart); }
...
function renderProductsGrid({categoryId=null, q=""}={}){
  ...
  if(!prods.length){ grid.innerHTML="<p class='center'>No products to show.</p>"; return; }
  ...
  qsa('[data-add]').forEach(btn=>btn.onclick=()=>{addToCart(btn.getAttribute('data-add')); alert("Added to cart");});
}
...
function renderProductDetail(){
  ...
  if(!id){ wrap.innerHTML="<p>Invalid product id</p>"; return; }
  ...
  byId("btn-add-cart").onclick = ()=>{ addToCart(p.id); alert("Added to cart"); };
}
...
function admin_showAllProducts(){
  ...
  qsa("[data-delete]").forEach(btn=>btn.onclick=()=>{ if(confirm("Delete?")){ deleteProduct(btn.getAttribute("data-delete")); admin_showAllProducts(); }});
  qsa("[data-edit]").forEach(btn=>btn.onclick=()=>admin_renderEditProductForm(btn.getAttribute("data-edit")));
}

function admin_renderAddProductForm(){
  const area = byId("admin-area");
  const opts = getCategories().map(c=>`<option value="${c.id}">${c.name}</option>`).join("");
  area.innerHTML=`
    <h2>Add Product</h2>
  ...
  byId("p-save").onclick=()=>{
    ...
    const images=byId("p-images").value.split(",").map(s=>s.trim()).filter(Boolean);
    const desc=byId("p-desc").value.trim();
    if(!title || !price || !catId) return alert("Title, price, category required");
...
function admin_renderEditProductForm(prodId){
  ...
  const opts=getCategories().map(c=>`<option value="${c.id}" ${p.category?.id==c.id?"selected":""}>${c.name}</option>`).join("");
  const area=byId("admin-area");
  area.innerHTML=`
    <h2>Edit Product</h2>
    <div class="form-row"><input id="p-title" value="${p.title}"/><input id="p-price" value="${p.price}" type="number"/></div>
    <div class="form-row"><select id="p-category">${opts}</select><input id="p-images" value="${p.images.join(",")}"/></div>
    <div class="form-row"><textarea id="p-desc" rows="4" style="width:100%">${p.description}</textarea></div>
    <div class="form-row"><button id="p-save" class="btn">Save</button><button id="p-cancel" class="btn secondary">Cancel</button></div>
  `;
  byId("p-save").onclick=()=>{
    const title=byId("p-title").value.trim();
    ...
    const desc=byId("p-desc").value.trim();
    if(!title || !price || !catId) return alert("Title, price, category required");
    updateProduct(p.id,{title, price, categoryId:catId, images, description:desc});
    admin_showAllProducts();
  };
  byId("p-cancel").onclick=admin_showAllProducts;
}
...
function admin_showAllCategories() {
  ...
  getCategories().forEach(c => {
    ...
    card.className = "card";
    card.innerHTML = `
      <img src="${c.image || 'https://via.placeholder.com/100'}" alt="${c.name}" style="width:100px;height:100px;object-fit:cover" />
      <h3>${c.name}</h3>
      <div style="display:flex;gap:8px;margin-top:8px;">
    ...
    // EDIT button
    card.querySelector("[data-edit]").onclick = () => admin_renderEditCategoryForm(c.id);

    // DELETE button
    card.querySelector("[data-delete]").onclick = () => {
      if(confirm("Delete category? All products in this category will also be deleted.")){
        deleteCategory(c.id);
...
function renderCartPage(){
  ...
  if(!cart.length){ container.innerHTML="<p>Your cart is empty 😕</p>"; totalElement.textContent=""; checkoutButton.style.display="none"; return; }
...
async function boot(){
  ...
  if(byId("products-grid")){
    ...
    select.onchange=()=>renderProductsGrid({categoryId:select.value||null, q:search?.value||""});
    search?.addEventListener("input",()=>renderProductsGrid({categoryId:select.value||null, q:search.value}));
  }

  if(byId("product-detail")) renderProductDetail();
  if(byId("admin-area")){
  byId("box-all-products")?.addEventListener("click", admin_showAllProducts);
  byId("box-all-categories")?.addEventListener("click", admin_showAllCategories); // <- add this
